/*
 * WolfVIS A.12 — Sprite scaler (per-column post-walk fixed-point).
 *
 * Adds DrawSpriteScaled, the last subsystem before the raycaster (A.13).
 * Same column-by-column pattern the raycaster will need, so this is the
 * seed for the wall-strip renderer too.
 *
 * Algorithm: for each destination column dx in [xc - h/2, xc + h/2),
 *   srcx = ((dx - dest_left) * 64) / h
 * If srcx is in [leftpix..rightpix], walk the source column's t_compshape
 * post triples (endy*2, corr_top, starty*2). For each opaque post, map
 * the source y range [starty..endy) onto [dy_start..dy_end) via the same
 * fixed-point ratio and write framebuf one pixel per dest row. Pure 8bpp
 * chunky writes — no plane-mask, no compiled scalers (the original Wolf3D
 * SimpleScaleShape JIT path is EGA/VGA-planar and irrelevant here).
 *
 * Layout change vs A.11: the center sprite slot (x=125, idx 1 = DeathCam)
 * is now dynamic — its current scale is g_scale (16..160, step 8 via A/B).
 * Side sprites (idx 0, 2) stay 1:1 in static_bg as size reference. Static
 * layer therefore omits the center sprite; on every WM_KEYDOWN we erase
 * both the cursor's previous footprint AND the scaled sprite's previous
 * bbox via RestoreFromBg, then redraw both at current state. Union of all
 * four rects (cursor prev/curr + scale prev/curr) is invalidated.
 *
 ...
 *   y=105..  : sides 1:1 (x=15, 235); center scaled around xc=157
 *
 * Controls (HC1 hand controller):
 *   d-pad        : move cursor
 *   F1           : init OPL3 + start music (combined one-touch)
 *   F3           : stop music
 *   A (PRIMARY)  : scale +8 (max 160) + high click tone
 *   B (SECONDARY): scale -8 (min 16)  + mid click tone
 *
 * --- A.10.1 polish (S6 follow-up, kept) ---
 *   - PIT-direct hi-res clock (852 cycles/IMF tick on MAME-VIS).
 *   - skip-the-gap: snap alTimeCount back when burst-drain would compress.
 *   - service-after-dispatch in WinMain.
 *
 * --- A.11 baseline (kept) ---
 *   walls + minimap + sprites + cursor + click tones + IMF music in one
 *   composited frame. Channel separation: OPL ch0 click coexists with IMF
 *   ch1..8 stream. __far placement for walls[]/sprites[] keeps DGROUP
 *   under 64 KB.
 */
#include <conio.h>
#include <windows.h>
...
/* A.12 sprite scaler state. Center slot (sprite idx 1, DeathCam) is
 * dynamic; A grows scale, B shrinks. xc is fixed at 157 (middle of the
 * old SPRITE_X1=125..189 cell), yc_top anchors at SPRITE_TOP_Y=105.
 * g_scale is the destination height in pixels; dest is square. */
#define SCALE_MIN  16
#define SCALE_MAX  160
#define SCALE_STEP 8
static int   g_scale         = 64;        /* start at 1:1 */
static int   g_scale_xc      = 157;
static int   g_scale_yc      = 105;
static RECT  g_scale_prev_rc = { 125, 105, 189, 169 };  /* matches initial 64×64 at xc=157 */
...
/* A.12: per-column post-walk fixed-point scaler.
 *
 * Side length = scale_h pixels. Sprites are 64×64 in source. Same
 * t_compshape format DrawSprite consumes; no decompression buffer.
 * The two long divisions per pixel are not free on a 286, but for a
 * single sprite at PoC scale (≤160 px) the cost is well under one
 * paint frame. The raycaster will reuse this exact column-walk shape
 * with a per-column height fed by the wall cast — A.13 leverage. */
static void DrawSpriteScaled(int xc, int yc_top, int idx, int scale_h)
{
    BYTE *sprite;
    WORD leftpix, rightpix;
    WORD far *dataofs;
    ...
    int  dx, dy, srcx, sy_src;
    int  dest_left, dest_right, dy_start, dy_end, fb_y;

    if (sprite_len[idx] == 0) return;
    if (scale_h < 2 || scale_h > 320) return;
    ...
    dest_left  = xc - scale_h / 2;
    dest_right = dest_left + scale_h;

    for (dx = dest_left; dx < dest_right; dx++) {
        if (dx < 0 || dx >= SCR_W) continue;
        srcx = (int)(((long)(dx - dest_left) * 64L) / (long)scale_h);
        ...
        while (post[0] != 0) {
            ...
            dy_start = yc_top + (int)(((long)starty_src * (long)scale_h) / 64L);
            dy_end   = yc_top + (int)(((long)endy_src   * (long)scale_h) / 64L);

            for (dy = dy_start; dy < dy_end; dy++) {
                if (dy < 0 || dy >= SCR_H) continue;
                sy_src = (int)(((long)(dy - yc_top) * 64L) / (long)scale_h);
                /* Clamp to current post bounds — guards against integer
                 * round-up at the segment edge feeding a wrong corr_top. */
                ...
                if (src_idx >= SPRITE_MAX) continue;
                fb_y = (SCR_H - 1) - dy;
                framebuf[(unsigned)fb_y * (unsigned)SCR_W + (unsigned)dx] = sprite[src_idx];
            }
            post += 3;
        }
    }
...
static void ScaleRect(RECT *r, int xc, int yc_top, int scale_h)
{
    r->left   = xc - scale_h / 2;
    r->top    = yc_top;
    r->right  = r->left + scale_h;
    r->bottom = yc_top  + scale_h;
}
...
static void SetupStaticBg(void)
{
    ...
    if (gVSwapErr == 0) {
        DrawWallStrip();
        /* Sides only — center slot (idx 1) is dynamic in A.12. */
...
long FAR PASCAL _export WolfVisWndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_KEYDOWN:
    case WM_SYSKEYDOWN: {
        RECT scale_cur, dirty;
        RECT cursor_prev_rc, cursor_cur_rc;
        ...
        case VK_HC1_PRIMARY:
            /* A.12: scale +SCALE_STEP. Click tone kept on ch0 (music ch1..8
             * unaffected — A.11 channel-separation pattern). */
            g_scale += SCALE_STEP;
            if (g_scale > SCALE_MAX) g_scale = SCALE_MAX;
            OplNoteOn(0x300, 5);
            break;
        case VK_HC1_SECONDARY:
            /* A.12: scale -SCALE_STEP + mid click + key-off. */
            g_scale -= SCALE_STEP;
        ...
        /* Erase prev footprints of both dynamic layers. The order matters
         * only if their bboxes overlap — RestoreFromBg writes from
         * static_bg, which contains neither, so two separate restores
         * are independent. */
        RestoreFromBg(g_scale_prev_rc.left, g_scale_prev_rc.top,
                      g_scale_prev_rc.right  - g_scale_prev_rc.left,
                      g_scale_prev_rc.bottom - g_scale_prev_rc.top);
        RestoreFromBg(cursor_prev_x - CURSOR_HALF,
                      cursor_prev_y - CURSOR_HALF,
                      CURSOR_HALF * 2 + 1,
                      CURSOR_HALF * 2 + 1);

        /* Redraw scaled sprite first, cursor on top — cursor is the
         * highest visual layer so it can ride over the sprite without
         * being obscured. */
        DrawSpriteScaled(g_scale_xc, g_scale_yc, 1, g_scale);
        DrawCursor(cursor_x, cursor_y);

        ScaleRect(&scale_cur, g_scale_xc, g_scale_yc, g_scale);
        cursor_prev_rc.left   = cursor_prev_x - CURSOR_HALF;
        cursor_prev_rc.top    = cursor_prev_y - CURSOR_HALF;
        cursor_prev_rc.right  = cursor_prev_x + CURSOR_HALF + 1;
        cursor_prev_rc.bottom = cursor_prev_y + CURSOR_HALF + 1;
        cursor_cur_rc.left    = cursor_x - CURSOR_HALF;
        cursor_cur_rc.top     = cursor_y - CURSOR_HALF;
        cursor_cur_rc.right   = cursor_x + CURSOR_HALF + 1;
        cursor_cur_rc.bottom  = cursor_y + CURSOR_HALF + 1;

        /* Union of all four rects. Single InvalidateRect covers the
         * combined dirty region — GDI clips StretchDIBits writes. */
        dirty = g_scale_prev_rc;
        if (scale_cur.left      < dirty.left)   dirty.left   = scale_cur.left;
        if (scale_cur.top       < dirty.top)    dirty.top    = scale_cur.top;
        if (scale_cur.right     > dirty.right)  dirty.right  = scale_cur.right;
        if (scale_cur.bottom    > dirty.bottom) dirty.bottom = scale_cur.bottom;
        if (cursor_prev_rc.left   < dirty.left)   dirty.left   = cursor_prev_rc.left;
        if (cursor_prev_rc.top    < dirty.top)    dirty.top    = cursor_prev_rc.top;
        if (cursor_prev_rc.right  > dirty.right)  dirty.right  = cursor_prev_rc.right;
        if (cursor_prev_rc.bottom > dirty.bottom) dirty.bottom = cursor_prev_rc.bottom;
        if (cursor_cur_rc.left    < dirty.left)   dirty.left   = cursor_cur_rc.left;
        if (cursor_cur_rc.top     < dirty.top)    dirty.top    = cursor_cur_rc.top;
        if (cursor_cur_rc.right   > dirty.right)  dirty.right  = cursor_cur_rc.right;
        if (cursor_cur_rc.bottom  > dirty.bottom) dirty.bottom = cursor_cur_rc.bottom;
...
int PASCAL WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR cmd, int show)
{
    ...
    SetupStaticBg();
    DrawDebugBar();
    /* A.12: initial scaled sprite at 1:1 in the center slot. */
    DrawSpriteScaled(g_scale_xc, g_scale_yc, 1, g_scale);
    ScaleRect(&g_scale_prev_rc, g_scale_xc, g_scale_yc, g_scale);
    DrawCursor(cursor_x, cursor_y);
    cursor_prev_x = cursor_x;
    cursor_prev_y = cursor_y;
    ...
    if (!hPrev) {
        ...
        wc.lpszClassName = "WolfVISa12";
        if (!RegisterClass(&wc)) return 0;
    }

    hWnd = CreateWindow(
        "WolfVISa12", "WolfVISa12",
        WS_POPUP | WS_VISIBLE,
        0, 0, 640, 480,
