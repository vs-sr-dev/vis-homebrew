/*
 * WolfVIS A.14 — Sprites in world (billboards over the raycaster scene).
 *
 * First milestone where the viewport is recognizably Wolfenstein 3D, not
 * just a tech demo of textured walls. Static decoration sprites placed at
 * their plane1 tile positions in E1L1, transformed world→camera each
 * frame, projected to viewport columns with focal=96 px, scaled by
 * inverse depth, and rendered after walls with a 1D per-column z-buffer
 * so walls correctly occlude near-side sprites.
 *
 * Adds vs A.13:
 *   - sprites[18][4096] __far re-loaded from VSWAP (Demo + DeathCam + 16
 *     SPR_STAT_*). LoadVSwap restores the A.5 sprite path.
 *   - Object[] / ScanObjects: scan plane1 for tile IDs 23..38 (Wolf3D
 *     static decoration object IDs); each match produces a billboard at
 *     tile center.
 *   - g_zbuffer[VIEW_W] long Q8.8: written by DrawWallStripCol per col.
 *   - DrawSpriteWorld: world→camera rotation, focal projection, sprite_h
 *     from inverse depth, per-column z-test vs g_zbuffer, scaled
 *     t_compshape sample (same column-walk pattern as A.12 / A.13).
 *   - DrawAllSprites called from DrawViewport after the wall loop.
 *
 * Layout 320x200:
...
static int IsWall(int tx, int ty)
{
    ...
    /* Wolf3D wall tiles: 1..63. Doors 90..101 also block the cast so they
     * render as walls. */
...
/* Movement-only collision: walls block, doors do NOT. Lets the player
 * walk through closed-door tiles (cosmetic glitch — they still render
 * as wall slabs since IsWall keeps blocking the cast — but A.14 ships
 * without door-open logic, and being stuck in the spawn cell with no
 * way out is a worse PoC experience than walking through a wall slab.
 * A.14.1 polish: real door swing animation + open-door render. */
...
int PASCAL WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR cmd, int show)
{
    ...
    if (!hPrev) {
        ...
        wc.lpszClassName = "WolfVISa14";
        if (!RegisterClass(&wc)) return 0;
    }

    hWnd = CreateWindow(
        "WolfVISa14", "WolfVISa14",
        WS_POPUP | WS_VISIBLE,
        0, 0, 640, 480,
