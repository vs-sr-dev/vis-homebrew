/*
 * WolfVIS A.8 — OPL3 smoke test.
 * A.7 baseline (GAMEMAPS minimap) + direct port I/O to OPL3 chip @ 0x388/0x389.
 * Minimum: init OPL2-compat mode, program channel 0 for sine A4, keyon, hold.
 * Debug bar shows "audio on" indicator.
 *
 * OPL3 register layout (VIS Yamaha YMF262):
 *   Address port 0x388 = register index (bank 0)
 *   Data port    0x389 = value
 *   Address port 0x38A = register index (bank 1, OPL3 only)
 *   Data port    0x38B = value (bank 1)
 *   Timing: after write to address port wait ~3 us (IN to 0x388 a few times);
 *   after data port write wait ~20 us.
 * Channel 0 operators: op0 (modulator) at reg_offset 0x00, op1 (carrier) at 0x03.
 *   0x20..0x35 = AM/VIB/EG/KSR + multiple
 *   0x40..0x55 = KSL + output level
 *   0x60..0x75 = attack/decay
 *   0x80..0x95 = sustain/release
 *   0xE0..0xF5 = waveform
 *   0xA0..0xA8 = F-num low (per channel)
 *   0xB0..0xB8 = key-on + block + F-num high (per channel)
 *   0xC0..0xC8 = feedback + algorithm (per channel)
 * F-num formula: F = freq_hz * 2^(20-block) / 49716. A4=440Hz, block=4 -> F=580 (0x244).
 */
#include <conio.h>
/*
 * WolfVIS A.7 baseline (kept for reference) — GAMEMAPS loader + minimap render.
...
static BYTE     gAudioOn = 0;     /* 0=off, 1=note playing */
...
long FAR PASCAL _export WolfVisWndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_SYSKEYDOWN:
        ...
        case VK_HC1_SECONDARY:
            ...
            break;
        case VK_HC1_F1:
            /* Init OPL (if not yet) and play A4 */
    ...
    case WM_TIMER: {
         ...
         * if optimizer would otherwise drop the import. */
        pt.x = 0; pt.y = 0;
...
int PASCAL WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR cmd, int show)
{
     ...
     * This isolates whether continuous OPL tone starves input emulation. */
    RedrawScene();
