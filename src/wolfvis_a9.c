/*
 * WolfVIS A.9 — perf refactor (static background + cursor erase/redraw +
 * dirty-rect partial blit).
 *
 * A.8 baseline (OPL3 + minimap + HC input + sprites + walls) suffered from
 * microfreeze on every WM_KEYDOWN: each keypress redrew the full 320x200
 * scene (ClearFrame 64KB + DrawMinimap ~16K FB writes + DrawDebugBar +
 * DrawCursor) and forced a full-screen StretchDIBits. Estimated 150-200 ms
 * per keypress on a 286 @ 12 MHz, user-visible as input lag.
 *
 * A.9 splits the scene into three layers:
...
long FAR PASCAL _export WolfVisWndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_TIMER: {
        ...
        /* Repaint debug bar (top DEBUG_BAR_H rows). Cursor lives below
         * y=35 so it never overlaps and doesn't need redrawing here. */
        DrawDebugBar();
        dirty.left   = 0;
        dirty.top    = 0;
        dirty.right  = SCR_W;
