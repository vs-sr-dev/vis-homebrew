/*
 * WolfVIS A.14.1 — Doors (state machine + slab render + PRIMARY toggle).
 *
...
/* VSWAP */
#define WALL_COUNT       8      /* 8 wall pages for per-tile texture variety */
#define DOOR_TEX_IDX     8      /* sentinel: tex_idx == WALL_COUNT means
...
#define DOOR_STEP         2     /* amt delta per WM_TIMER tick */
#define DOOR_BLOCK_AMT    56    /* < this -> blocks movement */
#define DOOR_DIR_IDLE     0
#define DOOR_DIR_OPENING  1
#define DOOR_DIR_CLOSING  2
/* 18 sprite chunks: 0=Demo, 1=DeathCam, 2..17=SPR_STAT_0..SPR_STAT_15.
 * Static decoration objects in E1L1 (lamps, columns, plants, food, etc.)
 * map to obj_id 23..38 via SPR_STAT_n = obj_id - 23, our sprite_idx =
 * obj_id - 23 + 2 (the +2 skips the leading Demo/DeathCam slots). */
...
/* sprites[] = 18 * 4096 = 72 KB exceeds the 64 KB segment cap that
...
static int LoadVSwap(void)
{
    ...
    /* Sprites: NUM_SPRITES chunks starting at sprite_start_idx. Pages may
...
/* Advance every door's animation by one DOOR_STEP. Called from WM_TIMER
 * (50 ms cadence) so a full open/close transition takes 64/2 * 50 ms =
 * 1.6 s. Returns TRUE if any door changed state (so the caller can
 * trigger a viewport invalidate). Iterates only over door tiles (sparse
 * sweep) — we still scan the whole 4096 tiles which is fast at this
 * size; an Object[]-style index could replace the sweep if doors ever
 * become a perf hotspot. */
static BOOL AdvanceDoors(void)
{
    ...
    for (i = 0; i < MAP_TILES; i++) {
        ...
        if (g_door_dir[i] == DOOR_DIR_OPENING) {
            if (g_door_amt[i] >= DOOR_AMT_OPEN - DOOR_STEP) {
                g_door_amt[i] = DOOR_AMT_OPEN;
                g_door_dir[i] = DOOR_DIR_IDLE;
            } else {
                g_door_amt[i] = (BYTE)(g_door_amt[i] + DOOR_STEP);
            }
            changed = TRUE;
        } else if (g_door_dir[i] == DOOR_DIR_CLOSING) {
            if (g_door_amt[i] <= DOOR_STEP) {
                g_door_amt[i] = 0;
                g_door_dir[i] = DOOR_DIR_IDLE;
            } else {
                g_door_amt[i] = (BYTE)(g_door_amt[i] - DOOR_STEP);
...
static void ScanObjects(void)
{
    ...
    for (ty = 0; ty < MAP_H && g_num_objects < MAX_OBJECTS; ty++) {
        for (tx = 0; tx < MAP_W && g_num_objects < MAX_OBJECTS; tx++) {
            obj = map_plane1[ty * MAP_W + tx];
            if (obj < STAT_OBJ_FIRST || obj > STAT_OBJ_LAST) continue;
...
static long CastRay(int ra, int *out_tex_idx, int *out_tex_x)
{
    ...
    long last_pos_x_q16, last_pos_y_q16;
    WORD hit_tile;
    ...
    *out_tex_x   = 0;
    ...
    for (steps = 0; steps < MAX_CAST_STEPS; steps++) {
        last_pos_x_q16 = pos_x_q16;
        last_pos_y_q16 = pos_y_q16;
        pos_x_q16 += sub_dx;
        pos_y_q16 += sub_dy;
        tx = (int)(pos_x_q16 >> 16);
        ty = (int)(pos_y_q16 >> 16);

        /* Door check: if we're currently traversing a door tile (prev was
         * a door) OR we just entered one, test for mid-plane crossing.
         * The slab sits at X=center for vertical doors / Y=center for
         * horizontal. If the ray crossed the slab plane this sub-step
         * AND the perp coord at the crossing is still inside the tile
         * AND the perp_frac is past the open extent -> HIT. Else the
         * ray passes through the open portion and casting continues
         * (the next iterations exit the door tile and may hit a wall
         * on the far side via the standard branch below). */
        {
            int  cur_door = IsDoor(prev_tx, prev_ty);
            int  new_door = (tx != prev_tx || ty != prev_ty) ? IsDoor(tx, ty) : 0;
            int  d_tx, d_ty, d_orient;
            long mid_q16, axis_prev, axis_pos;
            long perp_prev, perp_pos, perp_at_mid, perp_frac_q16;
            long denom, t_q16, amt_q16;
            int  perp_tile_expected;
            int  amt;
            long hit_x_q16, hit_y_q16, dx_to_hit, dy_to_hit;

            if (cur_door)      { d_tx = prev_tx; d_ty = prev_ty; d_orient = cur_door; }
            else if (new_door) { d_tx = tx;      d_ty = ty;      d_orient = new_door; }
            else                 d_orient = 0;

            if (d_orient) {
                if (d_orient == 1) {
                    mid_q16   = ((long)d_tx << 16) + 0x8000L;
                    axis_prev = last_pos_x_q16; axis_pos = pos_x_q16;
                    perp_prev = last_pos_y_q16; perp_pos = pos_y_q16;
                    perp_tile_expected = d_ty;
                } else {
                    mid_q16   = ((long)d_ty << 16) + 0x8000L;
                    axis_prev = last_pos_y_q16; axis_pos = pos_y_q16;
                    perp_prev = last_pos_x_q16; perp_pos = pos_x_q16;
                    perp_tile_expected = d_tx;
                }
                if ((axis_prev <  mid_q16 && axis_pos >= mid_q16) ||
                    (axis_prev >= mid_q16 && axis_pos <  mid_q16)) {
                    denom = axis_pos - axis_prev;
                    if (denom != 0) {
                        t_q16 = ((mid_q16 - axis_prev) << 16) / denom;
                        perp_at_mid = perp_prev +
                            (((perp_pos - perp_prev) * t_q16) >> 16);
                        if ((int)(perp_at_mid >> 16) == perp_tile_expected) {
                            perp_frac_q16 = perp_at_mid & 0xFFFFL;
                            amt = g_door_amt[d_ty * MAP_W + d_tx];
                            amt_q16 = ((long)amt << 16) / DOOR_AMT_OPEN;
                            if (perp_frac_q16 >= amt_q16) {
                                /* HIT slab */
                                if (d_orient == 1) {
                                    hit_x_q16 = mid_q16;
                                    hit_y_q16 = perp_at_mid;
                                } else {
                                    hit_x_q16 = perp_at_mid;
                                    hit_y_q16 = mid_q16;
                                }
                                *out_tex_idx = DOOR_TEX_IDX;
                                *out_tex_x = (int)((perp_frac_q16 * 64L) >> 16);
                                if (*out_tex_x < 0)  *out_tex_x = 0;
                                if (*out_tex_x > 63) *out_tex_x = 63;

                                dx_to_hit = hit_x_q16 - (g_px << 8);
                                dy_to_hit = hit_y_q16 - (g_py << 8);
                                if (d_orient == 1 && step_dx_q16 != 0) {
                                    dist_q88 = ((dx_to_hit >> 8) * 32767L) /
                                               (step_dx_q16 >> 1);
                                    if (dist_q88 < 0) dist_q88 = -dist_q88;
                                } else if (d_orient == 2 && step_dy_q16 != 0) {
                                    dist_q88 = ((dy_to_hit >> 8) * 32767L) /
                                               (step_dy_q16 >> 1);
                                    if (dist_q88 < 0) dist_q88 = -dist_q88;
                                } else {
                                    dist_q88 = (d_orient == 1)
                                        ? (dx_to_hit >> 8) : (dy_to_hit >> 8);
                                    if (dist_q88 < 0) dist_q88 = -dist_q88;
                                }
                                if (dist_q88 < 16) dist_q88 = 16;
                                return dist_q88;
                            }
                            /* perp_frac in open portion -> ray passes
                             * through; standard tile flow takes over. */
                        }
                    }
                }
            }
...
static void DrawWallStripCol(int col, long perp_dist_q88, int tex_idx, int tex_x)
{
    ...
    /* Z-buffer: store this column's wall depth so sprites can occlude. */
    ...
    /* DOOR_TEX_IDX (== WALL_COUNT) is the sentinel used by CastRay's
     * door branch — sample door_tex instead of the walls[] bank. */
    if (tex_idx == DOOR_TEX_IDX) {
        texcol = door_tex + ((unsigned)tex_x * 64U);
    } else {
...
int PASCAL WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR cmd, int show)
{
    ...
    if (!hPrev) {
        ...
        wc.lpszClassName = "WolfVISa141";
        if (!RegisterClass(&wc)) return 0;
    }

    hWnd = CreateWindow(
        "WolfVISa141", "WolfVISa141",
        WS_POPUP | WS_VISIBLE,
        0, 0, 640, 480,
